...
/* Display Log Message when Messages receive from or to Pyld1 */
#define PAYLOAD1_TX_RECEIVE   	1
#define PAYLOAD1_RX_RECEIVE   	1
...
int PyldMsgTx(const char * MsgQueueDescriptor, int txFd){
	...
	if(mqd < (mqd_t) 0)
	{
		syslog(LOG_INFO, "PayloadIfTx: Failed to open receive queue");
		LogErrno();
		return -1;
	}

	#if DEBUG_LEVEL > 15
		syslog(LOG_INFO, "PayloadIfTx: Cleared the Open ");
	#endif

	/* Get message attributes for allocating size */
	if(mq_getattr(mqd, &attr) == -1)
	{
		syslog(LOG_INFO,"PayloadIfTx: Failed to get message attributes");
		LogErrno();
		return -1;
	}

	#if DEBUG_LEVEL > 15
		syslog(LOG_INFO, "PayloadIfTx: Cleared the Attributes ");
	#endif

	ARM_char_t *ASCII_Buff = (ARM_char_t*) malloc(attr.mq_msgsize);

	if ( ASCII_Buff == NULL){
		syslog(LOG_INFO, "PayloadIfTx: Failed to allocate buffer ");
		return -1;
	}
	...
	if(numRead == -1){
		...
		syslog(LOG_INFO, "PayloadIfTx: Failed to read messages from Rx Queue ");
		LogErrno();
		return -1;
	}

	#if DEBUG_LEVEL > 15
		syslog(LOG_INFO, "PayloadIfTx: Cleared the mqreceive  ");
	#endif
	...
	#if DEBUG_LEVEL > 20
		syslog(LOG_INFO, "PayloadIfTx: Write Cleared");
	#endif
	...
	if(mq_close(mqd) < (int32_t)0){
			syslog(LOG_INFO, "PayloadIfTx: Close Failed");
			LogErrno();
	}

	#if DEBUG_LEVEL > 20
		syslog(LOG_INFO, "PayloadIfTx: Cleared the mq_close");
	#endif
...
/* This function receives serial data from the payload, and places
 * it an outgoing Message Queue */
int
PyldMsgRx( int ttyFdPyldn, char * RxMsgQueue ){
	...
	while ( !done)  {
		TotalRxBytes=read(ttyFdPyldn, ReadBuff, MAX_READ_BYTES);
	...
	/* Get Current System Time, copy it to our serial packet header */
		...
		if(mqd == (mqd_t) -1)
		{
			syslog(LOG_INFO, "ERROR : PyldMsgRx: Message Queue Open Fail");
			LogErrno();
			return -1;
...
main()
{
    ...
    int32_t ttyFdPyld1 = 0, ttyFdPyld2 = 0, ttyFdPyld3 = 0,  ttyFdPyld4 = 0;
    ...
    char UsrMsg[300];
		...
		openlog(PYLD_DAEMON_LOG_NAME, LOG_CONS | LOG_NDELAY | LOG_PERROR | LOG_PID, LOG_USER );
    ...
    MsgQueueRxInit(MQTXPYLD1);
    /* MsgQueueRxInit(MQRTPYLD2); */
    /* MsgQueueRxInit(MQRTPYLD3); */
    /* MsgQueueRxInit(MQRtPYLD4); */

    /* Open the file descriptor version of the message queue */
    MqFd1=open(PATHMQTXPYLD1 ,O_RDONLY);

    if(MqFd1== -1)
    {
    	syslog(LOG_INFO, "ERROR: Payload 1 Outgoing Message Queue Open (File Descriptor)" );
    	LogErrno();
    }

    /* MqFd2=open(PATHMQTXPYLD2 ,O_RDONLY);

    if(MqFd2== -1)
    {
    	syslog(LOG_INFO, "ERROR: Payload 2 Outgoing Message Queue Open (File Descriptor)" );
    	LogErrno();
    }

    MqFd3=open(PATHMQTXPYLD3 ,O_RDONLY);

    if(MqFd3== -1)
    {
    	syslog(LOG_INFO, "ERROR: Payload 3 Outgoing Message Queue Open (File Descriptor)" );
    	LogErrno();
    }

    MqFd4=open(PATHMQTXPYLD4 ,O_RDONLY);

    if(MqFd4== -1)
    {
    	syslog(LOG_INFO, "ERROR: Payload 4 Outgoing Message Queue Open (File Descriptor)" );
    	LogErrno();
    }
    ...
    {
        syslog(LOG_INFO, "ERROR:Message Queue 1 File DescriPyldMsgRx( ttyFdPyld1, MQRXPYLD1 );ptor EPOLL ADD Failed");
        LogErrno();
    }

    /* Open the Serial Drivers   */
    ttyFdPyld1=SerialConfigure(TTYPYLD1, TTYUSB0BAUDRATE);

    if(ttyFdPyld1== -1)
    {
    	syslog(LOG_INFO, "ERROR: Payload 1 Serial File Port Open Failed" );
    }

    ev.events = EPOLLIN;            /* Only interested in input events */
    ev.data.fd = ttyFdPyld1;

    if (epoll_ctl(epfd, EPOLL_CTL_ADD, ttyFdPyld1, &ev) == -1)
    {
        syslog(LOG_INFO, "ERROR: Payload 1 File Descriptor Epoll add failed");
        LogErrno();
    }
       ...
       /* 	PyldMsgTx(MQTXPYLD1, TTYPYLD1); */
        ...
        for (j = 0; j < ready; j++) {
            ...
            if (evlist[j].events & EPOLLIN) {
            	/* If a Tx Message to the Payload (via Message Queue on File System)
            	 * transmit this message to the serial driver. */
            	if(evlist[j].data.fd == MqFd1 )
            	{
					#if PAYLOAD1_TX_RECEIVE == 1
            			syslog(LOG_INFO, "Message Received for Pyld1");
					#endif

            		returnVal=PyldMsgTx(MQTXPYLD1, ttyFdPyld1 );

            		if(returnVal < 0)
            		{
            			syslog(LOG_INFO, "ERROR: Failed to transmit message to Payload 1");
            			continue;
            		}
            	}

            	/* If a Rx Message from the Payload (via ttyUSBn)
            	 * place this message in queue. */
            	if(evlist[j].data.fd == ttyFdPyld1 )
            	{
					#if PAYLOAD1_RX_RECEIVE == 1
            			syslog(LOG_INFO, "Message Received from Pyld1");
					#endif

            		returnVal=PyldMsgRx( ttyFdPyld1, MQRXPYLD1 );

            		if(returnVal < 0)
            		{
            			syslog(LOG_INFO, "ERROR: Failed to receive message from Payload 1");
            			continue;
            		}
            	}
